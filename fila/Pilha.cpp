...
TipoCel *Top; //Variável utilizada para manter o controle do estado atual da pilha
                  //Como a pilha é implementada em memória dinâmica,
                  //Topo deve ser um ponteiro para a célula no topo da Pilha

int Cont;  //Variável para contar o número de elementos armazenados na Pilha
...
void Pilha_Construtor()
{
  ...
  Top = NULL;  //O construtor da Pilha deve iniciar uma Pilha vazia, sem memória alocada
                //para essa estrutura de dados (não há ainda elementos armazenados)
                //Portanto, o ponteiro Topo deve apontar para NULL

  Cont = 0;  //Inicia a variável para indicar a quantidade de elementos armazenados na Pilha
}
...
void Pilha_Destrutor()
{
   ...
   TipoCel *Tempo;  //Ponteiro temporário utilizado para desalocar a memória

   while(Top != NULL)  //Enquanto Topo não chegar em Nulo (Enquanto a Pilha não estiver vazia)
   {
     Tempo = Top;  //o ponteiro temporário deve apontar para o mesmo endereço que Topo
     Top = Top -> Prox; //Topo avança para a célula seguinte

     Tempo->Prox = NULL; //Para quebrar o vínculo que existia entre as células da Pilha (segurança)
     free(Tempo);  //desaloca o espaço de memória (célula) apontado por Temp
   }
...
bool Pilha_Vazia()
{
   return Top == NULL;   //Pela forma como os elementos são Empilhados e Desempilhados (métodos a seguir)
...
int Pilha_Tamanho()
{
    ...
    return Cont;   //É retornado o contador de elementos, manipulado nos métodos de Empilhar e Desempilhar
}
...
bool Pilha_Empilhar(TipoDado Elementos)
{

   TipoCel *Nov;  //Ponteiro temporário utilizado para apontar para a nova célula a ser criada
   Nov = (TipoCel *) malloc(sizeof(TipoCel));

   if(Nov==NULL)  //tenta alocar dinâmicamente na memória umam nova célula da Pilha
   ...
   {
       Nov->Item = Elementos;  //Como Nova aponta para um espaço de memória disponível, guardamos o Elemento a ser armazenado nesse espaço
       Nov->Prox = Top;      //Para fazer a ligação entre as células da Pilha, o novo espaço de memória reservado
       ...
       Top = Nov;            //Como a Pilha possui agora um novo topo apontado por Nova e o ponteiro Topo está um nível abaixo
                               //deve-se fazer com que Topo aponte para a nova célula criada

       Cont++;             //Como foi armazenado um novo Elemento, o contador deve ser incrementado
...
bool Pilha_Desempilhar(TipoDado &Elementos)
{

   if(!Pilha_Vazia())  //primeiro deve-se conferir se a Pilha não está vazia
   {

      TipoCel *Tempo = Top;  //Cria um ponteiro temporário (Temp) e faz apontar para o mesmo endereço de Topo
                                //Essa célula será excluída da Pilha

      Top = Top->Prox; //Faz o topo apontar para a célula seguinte
                         //Esse procedimento define a célula eleita como topo da Pilha

      Elementos = Tempo->Item;  //O Elemento a ser retornado está na célula que será excluída
                              //Esse elemento é o último que foi empilhado (inserido na Pilha)

      Tempo->Prox = NULL;  //Remove a ligação entre a célula a ser excluída e a célula de topo da Pilha (segurança)

      free(Tempo);         //Desaloca o espaço de memória reservado pela célula apontada por Temp
                          //(devolve esse espaço de memória ao sistema)

      Cont--;          //Como foi removido um Elemento, o contador deve ser decrementado
...
bool Pilha_Topo(TipoDado &Elementos)
{

     if(!Pilha_Vazia())  //primeiro deve-se conferir se a Pilha não está vazia
     {
        Elementos = Top->Item;  //Topo aponta uma célula com o último elemento empilhado
